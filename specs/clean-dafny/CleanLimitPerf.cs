...
namespace CleanLimitEngine {
  ...
  public partial class CleanEngine {
    ...
    public void ProcessItemWhenReady(ThunderDbStack._IStreamItem item, out Dafny.ISequence<ThunderDbStack._IDownstreamEvent> events, out BigInteger queryId)
    {
      events = Dafny.Sequence<ThunderDbStack._IDownstreamEvent>.Empty;
      queryId = BigInteger.Zero;
      if ((this).Ready()) {
        Dafny.ISequence<ThunderDbStack._IDownstreamEvent> _out0;
        BigInteger _out1;
        (this).ProcessItem(item, out _out0, out _out1);
        events = _out0;
        queryId = _out1;
      } else {
        events = Dafny.Sequence<ThunderDbStack._IDownstreamEvent>.FromElements();
