...
module CleanLimitParity {
  ...
  method Main(_noArgsParameter: seq<seq<char>>)
    decreases *
  {
    var runner := new CleanParityRunner();
    runner.RunScenario(""clean-small"", 1, 8, 4, 4, 2, 1, 1, 3, 2);
    var runner2 := new CleanParityRunner();
    runner2.RunScenario(""clean-medium"", 17, 20, 8, 8, 3, 2, 2, 5, 3);
  }
  ...
  class CleanParityRunner {
    var engine: CleanEngine

    constructor ()
      ensures this.engine.Ready()
      ensures fresh(this.engine)
    {
      this.engine := new CleanEngine();
    }

    method CheckState(name: string, opIndex: nat, item: StreamItem, state: EngineState, functionalEvents: seq<DownstreamEvent>, cleanEvents: seq<DownstreamEvent>, functionalQueryId: QueryId, cleanQueryId: QueryId)
      requires this.engine.Ready()
      decreases *
    {
      var mismatch := functionalQueryId != cleanQueryId || NormalizeEvents(functionalEvents) != NormalizeEvents(cleanEvents) || state.store != this.engine.store || state.docIds != this.engine.docIds || Entries(state.treap) != Entries(this.engine.docs) || state.nextQueryId != this.engine.nextQueryId || |state.queries| != |this.engine.queries|;
      if mismatch {
        print ""scenario "";
        print name;
        ...
        print ""\nclean entries="";
        print Entries(this.engine.docs);
        print ""\n"";
      }
      var i := 0;
        ...
        decreases |state.queries| - i
      {
        ...
        var cleanVisible := this.engine.QueryVisible(query.id);
        if !(query.id in this.engine.queries) || this.engine.queries[query.id].spec != query.spec || cleanVisible != query.visible {
          print ""scenario "";
          print name;
      ...
      decreases *
    {
      ...
      assert this.engine.Ready();
      var nextState, functionalEvents, functionalQueryId := ProcessItem(state, seedItem);
      var cleanEvents, cleanQueryId := this.engine.ProcessItem(seedItem);
      state := nextState;
      summary := UpdateSummary(summary, functionalEvents);
      this.CheckState(name, opIndex, seedItem, state, functionalEvents, cleanEvents, functionalQueryId, cleanQueryId);
      i := 0;
      while i < customers
        ...
        modifies this, this.engine
      {
        ...
        var cleanAddEvents, cleanAddQueryId := this.engine.ProcessItem(item);
        state := stateAfterAdd;
        summary := UpdateSummary(summary, functionalAddEvents);
        opIndex := opIndex + 1;
        this.CheckState(name, opIndex, item, state, functionalAddEvents, cleanAddEvents, functionalAddQueryId, cleanAddQueryId);
        i := i + 1;
      }
      var tick := 0;
      while tick < ticks
        ...
        modifies this, this.engine
      {
            ...
            case {:split false} HasState(docState) =>
              {
                ...
                var cleanUpdateEvents, cleanUpdateQueryId := this.engine.ProcessItem(item);
                state := stateAfterUpdate;
                summary := UpdateSummary(summary, functionalUpdateEvents);
                opIndex := opIndex + 1;
                this.CheckState(name, opIndex, item, state, functionalUpdateEvents, cleanUpdateEvents, functionalUpdateQueryId, cleanUpdateQueryId);
              }
          }
          update := update + 1;
        }
          ...
          modifies this, this.engine
        {
          ...
          var cleanInsertEvents, cleanInsertQueryId := this.engine.ProcessItem(item);
          state := stateAfterInsert;
          summary := UpdateSummary(summary, functionalInsertEvents);
          nextDocId := nextDocId + 1;
          opIndex := opIndex + 1;
          this.CheckState(name, opIndex, item, state, functionalInsertEvents, cleanInsertEvents, functionalInsertQueryId, cleanInsertQueryId);
          insert := insert + 1;
        }
        var delete := 0;
            ...
            case {:split false} HasState(docState) =>
              {
                ...
                var cleanDeleteEvents, cleanDeleteQueryId := this.engine.ProcessItem(item);
                state := stateAfterDelete;
                summary := UpdateSummary(summary, functionalDeleteEvents);
                opIndex := opIndex + 1;
                this.CheckState(name, opIndex, item, state, functionalDeleteEvents, cleanDeleteEvents, functionalDeleteQueryId, cleanDeleteQueryId);
              }
          }
          delete := delete + 1;
        }
...
module CleanLimitEngine {
  function DocVisibleInTreap(t: Treap, spec: QuerySpec, score: Score, id: DocId): bool
    ...
    decreases t, spec, score, id
  {
    if score < spec.minScore || score > spec.maxScore then
      false
    else
      var startRank: nat := TreapRank(t, spec.minScore, NoDoc); var docRank: nat := TreapRank(t, score, SomeDoc(id)); var upper: nat := TreapCountAtMost(t, spec.maxScore); docRank < upper && startRank <= docRank && docRank - startRank < spec.limit
  }
  ...
  class CleanEngine {
    ...
    method CollectVisibleQueriesInTreap(t: Treap, id: DocId, state: DocState)
        returns (matches: seq<QueryId>)
      requires SumConsistent(t)
      decreases *
    {
      matches := [];
    ...
    {
      if queryId in this.queries {
        this.queries := map key: int {:trigger this.queries[key]} {:trigger key in this.queries} | key in this.queries && key != queryId :: this.queries[key];
...
module CleanPendingRuntime {
  ...
  {
    PendingState(state.byQuery, map key: int {:trigger state.byDoc[key]} {:trigger key in state.byDoc} | key in state.byDoc && key != docId :: state.byDoc[key], RemoveDocId(state.docOrder, docId))
  }

  function RegisterPending(state: PendingState, docId: DocId, queryId: QueryId): PendingState
    decreases state, docId, queryId
  {
    var docsForQuery: seq<DocId> := if queryId in state.byQuery then AppendDocIdIfMissing(state.byQuery[queryId], docId) else [docId];
    var queriesForDoc: seq<QueryId> := if docId in state.byDoc then AppendQueryIdUnique(state.byDoc[docId], queryId) else [queryId];
    PendingState(state.byQuery[queryId := docsForQuery], state.byDoc[docId := queriesForDoc], AppendDocIdIfMissing(state.docOrder, docId))
  }

  function CancelPending(state: PendingState, docId: DocId, queryId: QueryId): PendingState
    decreases state, docId, queryId
  {
    var byQuery2: map<int, seq<DocId>> := if queryId in state.byQuery && ContainsId(state.byQuery[queryId], docId) then var nextDocs: seq<DocId> := RemoveDocId(state.byQuery[queryId], docId); if |nextDocs| == 0 then map key: int {:trigger state.byQuery[key]} {:trigger key in state.byQuery} | key in state.byQuery && key != queryId :: state.byQuery[key] else state.byQuery[queryId := nextDocs] else state.byQuery;
    var byDoc2: map<int, seq<QueryId>> := if docId in state.byDoc && ContainsQueryId(state.byDoc[docId], queryId) then var nextQueries: seq<QueryId> := RemoveQueryId(state.byDoc[docId], queryId); if |nextQueries| == 0 then map key: int {:trigger state.byDoc[key]} {:trigger key in state.byDoc} | key in state.byDoc && key != docId :: state.byDoc[key] else state.byDoc[docId := nextQueries] else state.byDoc;
    var order2: seq<DocId> := if docId in byDoc2 then state.docOrder else RemoveDocId(state.docOrder, docId);
  ...
  function ResolvePendingForDoc(state: PendingState, docId: DocId): PendingState
    decreases state, docId
  {
    if !(docId in state.byDoc) then
      state
    else
      var queriesForDoc: seq<QueryId> := state.byDoc[docId]; var byQuery2: map<QueryId, seq<DocId>> := RemoveDocFromQueries(state.byQuery, queriesForDoc, docId); PendingState(byQuery2, map key: int {:trigger state.byDoc[key]} {:trigger key in state.byDoc} | key in state.byDoc && key != docId :: state.byDoc[key], RemoveDocId(state.docOrder, docId))
  }
    ...
    decreases |queries|
  {
    if |queries| == 0 then
      ...
      var nextByQuery: map<int, seq<DocId>> := if queries[0] in byQuery then var nextDocs: seq<DocId> := RemoveDocId(byQuery[queries[0]], docId); if |nextDocs| == 0 then map key: int {:trigger byQuery[key]} {:trigger key in byQuery} | key in byQuery && key != queries[0] :: byQuery[key] else byQuery[queries[0] := nextDocs] else byQuery; RemoveDocFromQueries(nextByQuery, queries[1..], docId)
  }
    ...
    decreases |order|
  {
    if |order| == 0 then
      []
    else if !(order[0] in byDoc) then
      BuildGroupedRetrievals(order[1..], byDoc, store)
    else
      match LookupState(store, order[0]) case NoState() => BuildGroupedRetrievals(order[1..], byDoc, store) case HasState(state) => [RetrievalDoc(order[0], state, byDoc[order[0]])] + BuildGroupedRetrievals(order[1..], byDoc, store)
  }
  ...
  datatype PendingState = PendingState(byQuery: map<QueryId, seq<DocId>>, byDoc: map<DocId, seq<QueryId>>, docOrder: seq<DocId>)
...
namespace CleanPendingRuntime {

  public partial class __default {
    public static CleanPendingRuntime._IPendingState EmptyPendingState() {
      return CleanPendingRuntime.PendingState.create(Dafny.Map<BigInteger, Dafny.ISequence<BigInteger>>.FromElements(), Dafny.Map<BigInteger, Dafny.ISequence<BigInteger>>.FromElements(), Dafny.Sequence<BigInteger>.FromElements());
    }
    ...
    public static CleanPendingRuntime._IPendingState RemovePendingDoc(CleanPendingRuntime._IPendingState state, BigInteger docId)
    {
      return CleanPendingRuntime.PendingState.create((state).dtor_byQuery, Dafny.Helpers.Id<Func<CleanPendingRuntime._IPendingState, BigInteger, Dafny.IMap<BigInteger,Dafny.ISequence<BigInteger>>>>((_0_state, _1_docId) => ((System.Func<Dafny.IMap<BigInteger,Dafny.ISequence<BigInteger>>>)(() => {
  var _coll0 = new System.Collections.Generic.List<Dafny.Pair<BigInteger,Dafny.ISequence<BigInteger>>>();
  foreach (BigInteger _compr_0 in ((_0_state).dtor_byDoc).Keys.Elements) {
    BigInteger _2_key = (BigInteger)_compr_0;
    if ((((_0_state).dtor_byDoc).Contains(_2_key)) && ((_2_key) != (_1_docId))) {
      _coll0.Add(new Dafny.Pair<BigInteger,Dafny.ISequence<BigInteger>>(_2_key, Dafny.Map<BigInteger, Dafny.ISequence<BigInteger>>.Select((_0_state).dtor_byDoc,_2_key)));
    }
  }
  return Dafny.Map<BigInteger,Dafny.ISequence<BigInteger>>.FromCollection(_coll0);
}))())(state, docId), ThunderDbStack.__default.RemoveDocId((state).dtor_docOrder, docId));
    }
    public static CleanPendingRuntime._IPendingState RegisterPending(CleanPendingRuntime._IPendingState state, BigInteger docId, BigInteger queryId)
    {
      Dafny.ISequence<BigInteger> _0_docsForQuery = ((((state).dtor_byQuery).Contains(queryId)) ? (ThunderDbStack.__default.AppendDocIdIfMissing(Dafny.Map<BigInteger, Dafny.ISequence<BigInteger>>.Select((state).dtor_byQuery,queryId), docId)) : (Dafny.Sequence<BigInteger>.FromElements(docId)));
      Dafny.ISequence<BigInteger> _1_queriesForDoc = ((((state).dtor_byDoc).Contains(docId)) ? (ThunderDbStack.__default.AppendQueryIdUnique(Dafny.Map<BigInteger, Dafny.ISequence<BigInteger>>.Select((state).dtor_byDoc,docId), queryId)) : (Dafny.Sequence<BigInteger>.FromElements(queryId)));
      return CleanPendingRuntime.PendingState.create(Dafny.Map<BigInteger, Dafny.ISequence<BigInteger>>.Update((state).dtor_byQuery, queryId, _0_docsForQuery), Dafny.Map<BigInteger, Dafny.ISequence<BigInteger>>.Update((state).dtor_byDoc, docId, _1_queriesForDoc), ThunderDbStack.__default.AppendDocIdIfMissing((state).dtor_docOrder, docId));
    }
    public static CleanPendingRuntime._IPendingState CancelPending(CleanPendingRuntime._IPendingState state, BigInteger docId, BigInteger queryId)
    {
      ...
      Dafny.IMap<BigInteger,Dafny.ISequence<BigInteger>> _0_byQuery2 = (((((state).dtor_byQuery).Contains(queryId)) && (ThunderDbStack.__default.ContainsId(Dafny.Map<BigInteger, Dafny.ISequence<BigInteger>>.Select((state).dtor_byQuery,queryId), docId))) ? (Dafny.Helpers.Let<Dafny.ISequence<BigInteger>, Dafny.IMap<BigInteger,Dafny.ISequence<BigInteger>>>(ThunderDbStack.__default.RemoveDocId(Dafny.Map<BigInteger, Dafny.ISequence<BigInteger>>.Select((state).dtor_byQuery,queryId), docId), _pat_let0_0 => Dafny.Helpers.Let<Dafny.ISequence<BigInteger>, Dafny.IMap<BigInteger,Dafny.ISequence<BigInteger>>>(_pat_let0_0, _1_nextDocs => (((new BigInteger((_1_nextDocs).Count)).Sign == 0) ? (Dafny.Helpers.Id<Func<CleanPendingRuntime._IPendingState, BigInteger, Dafny.IMap<BigInteger,Dafny.ISequence<BigInteger>>>>((_2_state, _3_queryId) => ((System.Func<Dafny.IMap<BigInteger,Dafny.ISequence<BigInteger>>>)(() => {
        var _coll0 = new System.Collections.Generic.List<Dafny.Pair<BigInteger,Dafny.ISequence<BigInteger>>>();
        foreach (BigInteger _compr_0 in ((_2_state).dtor_byQuery).Keys.Elements) {
          BigInteger _4_key = (BigInteger)_compr_0;
          if ((((_2_state).dtor_byQuery).Contains(_4_key)) && ((_4_key) != (_3_queryId))) {
            _coll0.Add(new Dafny.Pair<BigInteger,Dafny.ISequence<BigInteger>>(_4_key, Dafny.Map<BigInteger, Dafny.ISequence<BigInteger>>.Select((_2_state).dtor_byQuery,_4_key)));
          }
        }
        return Dafny.Map<BigInteger,Dafny.ISequence<BigInteger>>.FromCollection(_coll0);
      }))())(_pat_let_tv0, _pat_let_tv1)) : (Dafny.Map<BigInteger, Dafny.ISequence<BigInteger>>.Update((_pat_let_tv2).dtor_byQuery, _pat_let_tv3, _1_nextDocs)))))) : ((state).dtor_byQuery));
      Dafny.IMap<BigInteger,Dafny.ISequence<BigInteger>> _5_byDoc2 = (((((state).dtor_byDoc).Contains(docId)) && (ThunderDbStack.__default.ContainsQueryId(Dafny.Map<BigInteger, Dafny.ISequence<BigInteger>>.Select((state).dtor_byDoc,docId), queryId))) ? (Dafny.Helpers.Let<Dafny.ISequence<BigInteger>, Dafny.IMap<BigInteger,Dafny.ISequence<BigInteger>>>(CleanPendingRuntime.__default.RemoveQueryId(Dafny.Map<BigInteger, Dafny.ISequence<BigInteger>>.Select((state).dtor_byDoc,docId), queryId), _pat_let1_0 => Dafny.Helpers.Let<Dafny.ISequence<BigInteger>, Dafny.IMap<BigInteger,Dafny.ISequence<BigInteger>>>(_pat_let1_0, _6_nextQueries => (((new BigInteger((_6_nextQueries).Count)).Sign == 0) ? (Dafny.Helpers.Id<Func<CleanPendingRuntime._IPendingState, BigInteger, Dafny.IMap<BigInteger,Dafny.ISequence<BigInteger>>>>((_7_state, _8_docId) => ((System.Func<Dafny.IMap<BigInteger,Dafny.ISequence<BigInteger>>>)(() => {
        var _coll1 = new System.Collections.Generic.List<Dafny.Pair<BigInteger,Dafny.ISequence<BigInteger>>>();
        foreach (BigInteger _compr_1 in ((_7_state).dtor_byDoc).Keys.Elements) {
          BigInteger _9_key = (BigInteger)_compr_1;
          if ((((_7_state).dtor_byDoc).Contains(_9_key)) && ((_9_key) != (_8_docId))) {
            _coll1.Add(new Dafny.Pair<BigInteger,Dafny.ISequence<BigInteger>>(_9_key, Dafny.Map<BigInteger, Dafny.ISequence<BigInteger>>.Select((_7_state).dtor_byDoc,_9_key)));
          }
        }
        return Dafny.Map<BigInteger,Dafny.ISequence<BigInteger>>.FromCollection(_coll1);
      }))())(_pat_let_tv4, _pat_let_tv5)) : (Dafny.Map<BigInteger, Dafny.ISequence<BigInteger>>.Update((_pat_let_tv6).dtor_byDoc, _pat_let_tv7, _6_nextQueries)))))) : ((state).dtor_byDoc));
      Dafny.ISequence<BigInteger> _10_order2 = (((_5_byDoc2).Contains(docId)) ? ((state).dtor_docOrder) : (ThunderDbStack.__default.RemoveDocId((state).dtor_docOrder, docId)));
      return CleanPendingRuntime.PendingState.create(_0_byQuery2, _5_byDoc2, _10_order2);
    }
    public static CleanPendingRuntime._IPendingState ResolvePendingForDoc(CleanPendingRuntime._IPendingState state, BigInteger docId)
    {
      if (!(((state).dtor_byDoc).Contains(docId))) {
        return state;
      } else {
        Dafny.ISequence<BigInteger> _0_queriesForDoc = Dafny.Map<BigInteger, Dafny.ISequence<BigInteger>>.Select((state).dtor_byDoc,docId);
        Dafny.IMap<BigInteger,Dafny.ISequence<BigInteger>> _1_byQuery2 = CleanPendingRuntime.__default.RemoveDocFromQueries((state).dtor_byQuery, _0_queriesForDoc, docId);
        return CleanPendingRuntime.PendingState.create(_1_byQuery2, Dafny.Helpers.Id<Func<CleanPendingRuntime._IPendingState, BigInteger, Dafny.IMap<BigInteger,Dafny.ISequence<BigInteger>>>>((_2_state, _3_docId) => ((System.Func<Dafny.IMap<BigInteger,Dafny.ISequence<BigInteger>>>)(() => {
  var _coll0 = new System.Collections.Generic.List<Dafny.Pair<BigInteger,Dafny.ISequence<BigInteger>>>();
  foreach (BigInteger _compr_0 in ((_2_state).dtor_byDoc).Keys.Elements) {
    BigInteger _4_key = (BigInteger)_compr_0;
    if ((((_2_state).dtor_byDoc).Contains(_4_key)) && ((_4_key) != (_3_docId))) {
      _coll0.Add(new Dafny.Pair<BigInteger,Dafny.ISequence<BigInteger>>(_4_key, Dafny.Map<BigInteger, Dafny.ISequence<BigInteger>>.Select((_2_state).dtor_byDoc,_4_key)));
    }
  }
  return Dafny.Map<BigInteger,Dafny.ISequence<BigInteger>>.FromCollection(_coll0);
}))())(state, docId), ThunderDbStack.__default.RemoveDocId((state).dtor_docOrder, docId));
    ...
    public static Dafny.IMap<BigInteger,Dafny.ISequence<BigInteger>> RemoveDocFromQueries(Dafny.IMap<BigInteger,Dafny.ISequence<BigInteger>> byQuery, Dafny.ISequence<BigInteger> queries, BigInteger docId)
    {
    TAIL_CALL_START: ;
      var _pat_let_tv0 = byQuery;
      var _pat_let_tv1 = queries;
      var _pat_let_tv2 = byQuery;
      var _pat_let_tv3 = queries;
      if ((new BigInteger((queries).Count)).Sign == 0) {
        return byQuery;
      } else {
        Dafny.IMap<BigInteger,Dafny.ISequence<BigInteger>> _0_nextByQuery = (((byQuery).Contains((queries).Select(BigInteger.Zero))) ? (Dafny.Helpers.Let<Dafny.ISequence<BigInteger>, Dafny.IMap<BigInteger,Dafny.ISequence<BigInteger>>>(ThunderDbStack.__default.RemoveDocId(Dafny.Map<BigInteger, Dafny.ISequence<BigInteger>>.Select(byQuery,(queries).Select(BigInteger.Zero)), docId), _pat_let2_0 => Dafny.Helpers.Let<Dafny.ISequence<BigInteger>, Dafny.IMap<BigInteger,Dafny.ISequence<BigInteger>>>(_pat_let2_0, _1_nextDocs => (((new BigInteger((_1_nextDocs).Count)).Sign == 0) ? (Dafny.Helpers.Id<Func<Dafny.IMap<BigInteger,Dafny.ISequence<BigInteger>>, Dafny.ISequence<BigInteger>, Dafny.IMap<BigInteger,Dafny.ISequence<BigInteger>>>>((_2_byQuery, _3_queries) => ((System.Func<Dafny.IMap<BigInteger,Dafny.ISequence<BigInteger>>>)(() => {
          var _coll0 = new System.Collections.Generic.List<Dafny.Pair<BigInteger,Dafny.ISequence<BigInteger>>>();
          foreach (BigInteger _compr_0 in (_2_byQuery).Keys.Elements) {
            BigInteger _4_key = (BigInteger)_compr_0;
            if (((_2_byQuery).Contains(_4_key)) && ((_4_key) != ((_3_queries).Select(BigInteger.Zero)))) {
              _coll0.Add(new Dafny.Pair<BigInteger,Dafny.ISequence<BigInteger>>(_4_key, Dafny.Map<BigInteger, Dafny.ISequence<BigInteger>>.Select(_2_byQuery,_4_key)));
            }
          }
          return Dafny.Map<BigInteger,Dafny.ISequence<BigInteger>>.FromCollection(_coll0);
        }))())(_pat_let_tv0, _pat_let_tv1)) : (Dafny.Map<BigInteger, Dafny.ISequence<BigInteger>>.Update(_pat_let_tv2, (_pat_let_tv3).Select(BigInteger.Zero), _1_nextDocs)))))) : (byQuery));
        Dafny.IMap<BigInteger,Dafny.ISequence<BigInteger>> _in0 = _0_nextByQuery;
    ...
    public static Dafny.ISequence<ThunderDbStack._IRetrievalDoc> BuildGroupedRetrievals(Dafny.ISequence<BigInteger> order, Dafny.IMap<BigInteger,Dafny.ISequence<BigInteger>> byDoc, Dafny.IMap<BigInteger,BigInteger> store)
    {
      Dafny.ISequence<ThunderDbStack._IRetrievalDoc> _0___accumulator = Dafny.Sequence<ThunderDbStack._IRetrievalDoc>.FromElements();
    TAIL_CALL_START: ;
      if ((new BigInteger((order).Count)).Sign == 0) {
        return Dafny.Sequence<ThunderDbStack._IRetrievalDoc>.Concat(_0___accumulator, Dafny.Sequence<ThunderDbStack._IRetrievalDoc>.FromElements());
      } else if (!((byDoc).Contains((order).Select(BigInteger.Zero)))) {
        Dafny.ISequence<BigInteger> _in0 = (order).Drop(BigInteger.One);
        Dafny.IMap<BigInteger,Dafny.ISequence<BigInteger>> _in1 = byDoc;
        Dafny.IMap<BigInteger,BigInteger> _in2 = store;
        order = _in0;
        byDoc = _in1;
        store = _in2;
        ...
        ThunderDbStack._IMaybeDocState _source0 = ThunderDbStack.__default.LookupState(store, (order).Select(BigInteger.Zero));
        {
          if (_source0.is_NoState) {
            ...
            Dafny.IMap<BigInteger,Dafny.ISequence<BigInteger>> _in4 = byDoc;
        ...
        {
          BigInteger _1_state = _source0.dtor_state;
          return Dafny.Sequence<ThunderDbStack._IRetrievalDoc>.Concat(Dafny.Sequence<ThunderDbStack._IRetrievalDoc>.FromElements(ThunderDbStack.RetrievalDoc.create((order).Select(BigInteger.Zero), _1_state, Dafny.Map<BigInteger, Dafny.ISequence<BigInteger>>.Select(byDoc,(order).Select(BigInteger.Zero)))), CleanPendingRuntime.__default.BuildGroupedRetrievals((order).Drop(BigInteger.One), byDoc, store));
        }
      }
    }
  }

  public interface _IPendingState {
    bool is_PendingState { get; }
    Dafny.IMap<BigInteger,Dafny.ISequence<BigInteger>> dtor_byQuery { get; }
    Dafny.IMap<BigInteger,Dafny.ISequence<BigInteger>> dtor_byDoc { get; }
    Dafny.ISequence<BigInteger> dtor_docOrder { get; }
    _IPendingState DowncastClone();
  }
  public class PendingState : _IPendingState {
    public readonly Dafny.IMap<BigInteger,Dafny.ISequence<BigInteger>> _byQuery;
    public readonly Dafny.IMap<BigInteger,Dafny.ISequence<BigInteger>> _byDoc;
    public readonly Dafny.ISequence<BigInteger> _docOrder;
    public PendingState(Dafny.IMap<BigInteger,Dafny.ISequence<BigInteger>> byQuery, Dafny.IMap<BigInteger,Dafny.ISequence<BigInteger>> byDoc, Dafny.ISequence<BigInteger> docOrder) {
      this._byQuery = byQuery;
      this._byDoc = byDoc;
      this._docOrder = docOrder;
    }
    public _IPendingState DowncastClone() {
      if (this is _IPendingState dt) { return dt; }
      return new PendingState(_byQuery, _byDoc, _docOrder);
    }
    public override bool Equals(object other) {
      var oth = other as CleanPendingRuntime.PendingState;
      return oth != null && object.Equals(this._byQuery, oth._byQuery) && object.Equals(this._byDoc, oth._byDoc) && object.Equals(this._docOrder, oth._docOrder);
    }
    public override int GetHashCode() {
      ulong hash = 5381;
      hash = ((hash << 5) + hash) + 0;
      hash = ((hash << 5) + hash) + ((ulong)Dafny.Helpers.GetHashCode(this._byQuery));
      hash = ((hash << 5) + hash) + ((ulong)Dafny.Helpers.GetHashCode(this._byDoc));
      hash = ((hash << 5) + hash) + ((ulong)Dafny.Helpers.GetHashCode(this._docOrder));
      return (int) hash;
    }
    public override string ToString() {
      string s = "CleanPendingRuntime.PendingState.PendingState";
      s += "(";
      s += Dafny.Helpers.ToString(this._byQuery);
      s += ", ";
      s += Dafny.Helpers.ToString(this._byDoc);
      s += ", ";
      s += Dafny.Helpers.ToString(this._docOrder);
      s += ")";
      return s;
    }
    private static readonly CleanPendingRuntime._IPendingState theDefault = create(Dafny.Map<BigInteger, Dafny.ISequence<BigInteger>>.Empty, Dafny.Map<BigInteger, Dafny.ISequence<BigInteger>>.Empty, Dafny.Sequence<BigInteger>.Empty);
    public static CleanPendingRuntime._IPendingState Default() {
      return theDefault;
    }
    private static readonly Dafny.TypeDescriptor<CleanPendingRuntime._IPendingState> _TYPE = new Dafny.TypeDescriptor<CleanPendingRuntime._IPendingState>(CleanPendingRuntime.PendingState.Default());
    public static Dafny.TypeDescriptor<CleanPendingRuntime._IPendingState> _TypeDescriptor() {
      return _TYPE;
    }
    public static _IPendingState create(Dafny.IMap<BigInteger,Dafny.ISequence<BigInteger>> byQuery, Dafny.IMap<BigInteger,Dafny.ISequence<BigInteger>> byDoc, Dafny.ISequence<BigInteger> docOrder) {
      return new PendingState(byQuery, byDoc, docOrder);
    }
    public static _IPendingState create_PendingState(Dafny.IMap<BigInteger,Dafny.ISequence<BigInteger>> byQuery, Dafny.IMap<BigInteger,Dafny.ISequence<BigInteger>> byDoc, Dafny.ISequence<BigInteger> docOrder) {
      return create(byQuery, byDoc, docOrder);
    }
    public bool is_PendingState { get { return true; } }
    public Dafny.IMap<BigInteger,Dafny.ISequence<BigInteger>> dtor_byQuery {
      get {
        return this._byQuery;
      }
    }
    public Dafny.IMap<BigInteger,Dafny.ISequence<BigInteger>> dtor_byDoc {
      get {
        return this._byDoc;
      }
    }
    public Dafny.ISequence<BigInteger> dtor_docOrder {
      get {
  ...
  public partial class PendingRegistry {
    public PendingRegistry() {
      this.state = CleanPendingRuntime.PendingState.Default();
    }
    public CleanPendingRuntime._IPendingState state {get; set;}
    public void __ctor()
    {
      (this).state = CleanPendingRuntime.__default.EmptyPendingState();
    }
    public void Register(BigInteger docId, BigInteger queryId)
    {
      (this).state = CleanPendingRuntime.__default.RegisterPending(this.state, docId, queryId);
    }
    public bool Cancel(BigInteger docId, BigInteger queryId)
    {
      bool removed = false;
      removed = (((this.state).dtor_byDoc).Contains(docId)) && (ThunderDbStack.__default.ContainsQueryId(Dafny.Map<BigInteger, Dafny.ISequence<BigInteger>>.Select((this.state).dtor_byDoc,docId), queryId));
      (this).state = CleanPendingRuntime.__default.CancelPending(this.state, docId, queryId);
      return removed;
    }
    public void ResolveDoc(BigInteger docId)
    {
      (this).state = CleanPendingRuntime.__default.ResolvePendingForDoc(this.state, docId);
    }
    public Dafny.ISequence<BigInteger> PendingQueries(BigInteger docId)
    {
      Dafny.ISequence<BigInteger> queries = Dafny.Sequence<BigInteger>.Empty;
      if (((this.state).dtor_byDoc).Contains(docId)) {
        queries = Dafny.Map<BigInteger, Dafny.ISequence<BigInteger>>.Select((this.state).dtor_byDoc,docId);
      } else {
        queries = Dafny.Sequence<BigInteger>.FromElements();
      }
      return queries;
    }
    public Dafny.ISequence<BigInteger> PendingDocs(BigInteger queryId)
    {
      Dafny.ISequence<BigInteger> docs = Dafny.Sequence<BigInteger>.Empty;
      if (((this.state).dtor_byQuery).Contains(queryId)) {
        docs = Dafny.Map<BigInteger, Dafny.ISequence<BigInteger>>.Select((this.state).dtor_byQuery,queryId);
      } else {
        docs = Dafny.Sequence<BigInteger>.FromElements();
      }
      return docs;
    }
    public Dafny.ISequence<ThunderDbStack._IRetrievalDoc> DrainGrouped(Dafny.IMap<BigInteger,BigInteger> store)
    {
      Dafny.ISequence<ThunderDbStack._IRetrievalDoc> docs = Dafny.Sequence<ThunderDbStack._IRetrievalDoc>.Empty;
      docs = CleanPendingRuntime.__default.BuildGroupedRetrievals((this.state).dtor_docOrder, (this.state).dtor_byDoc, store);
      (this).state = CleanPendingRuntime.__default.EmptyPendingState();
      return docs;
    }
  }
} // end of namespace CleanPendingRuntime
namespace CleanLimitEngine {

  public partial class __default {
    public static bool DocVisibleInTreap(DocsIndexTreap._ITreap t, ThunderDbStack._IQuerySpec spec, BigInteger score, BigInteger id)
    {
      if (((score) < ((spec).dtor_minScore)) || ((score) > ((spec).dtor_maxScore))) {
        return false;
      } else {
        BigInteger _0_startRank = DocsIndexTreap.__default.TreapRank(t, (spec).dtor_minScore, DocsIndexModel.MaybeDocId.create_NoDoc());
        BigInteger _1_docRank = DocsIndexTreap.__default.TreapRank(t, score, DocsIndexModel.MaybeDocId.create_SomeDoc(id));
        BigInteger _2_upper = DocsIndexTreap.__default.TreapCountAtMost(t, (spec).dtor_maxScore);
        return (((_1_docRank) < (_2_upper)) && ((_0_startRank) <= (_1_docRank))) && (((_1_docRank) - (_0_startRank)) < ((spec).dtor_limit));
      }
    }
  }

  public interface _ICleanQueryState {
    bool is_CleanQueryState { get; }
    ThunderDbStack._IQuerySpec dtor_spec { get; }
    BigInteger dtor_currentMatches { get; }
    _ICleanQueryState DowncastClone();
  }
  public class CleanQueryState : _ICleanQueryState {
    public readonly ThunderDbStack._IQuerySpec _spec;
    public readonly BigInteger _currentMatches;
    public CleanQueryState(ThunderDbStack._IQuerySpec spec, BigInteger currentMatches) {
      this._spec = spec;
      this._currentMatches = currentMatches;
    }
    public _ICleanQueryState DowncastClone() {
      if (this is _ICleanQueryState dt) { return dt; }
      return new CleanQueryState(_spec, _currentMatches);
    }
    public override bool Equals(object other) {
      var oth = other as CleanLimitEngine.CleanQueryState;
      return oth != null && object.Equals(this._spec, oth._spec) && this._currentMatches == oth._currentMatches;
    }
    public override int GetHashCode() {
      ulong hash = 5381;
      hash = ((hash << 5) + hash) + 0;
    ...
    public override string ToString() {
      string s = "CleanLimitEngine.CleanQueryState.CleanQueryState";
      s += "(";
      s += Dafny.Helpers.ToString(this._spec);
      s += ", ";
    ...
    private static readonly CleanLimitEngine._ICleanQueryState theDefault = create(ThunderDbStack.QuerySpec.Default(), BigInteger.Zero);
    public static CleanLimitEngine._ICleanQueryState Default() {
      return theDefault;
    }
    private static readonly Dafny.TypeDescriptor<CleanLimitEngine._ICleanQueryState> _TYPE = new Dafny.TypeDescriptor<CleanLimitEngine._ICleanQueryState>(CleanLimitEngine.CleanQueryState.Default());
    public static Dafny.TypeDescriptor<CleanLimitEngine._ICleanQueryState> _TypeDescriptor() {
      return _TYPE;
    }
    public static _ICleanQueryState create(ThunderDbStack._IQuerySpec spec, BigInteger currentMatches) {
      return new CleanQueryState(spec, currentMatches);
    }
    public static _ICleanQueryState create_CleanQueryState(ThunderDbStack._IQuerySpec spec, BigInteger currentMatches) {
      return create(spec, currentMatches);
    }
    public bool is_CleanQueryState { get { return true; } }
    public ThunderDbStack._IQuerySpec dtor_spec {
      get {
        return this._spec;
      }
  ...
  public partial class CleanEngine {
    public CleanEngine() {
      ...
      this.queries = Dafny.Map<BigInteger, CleanLimitEngine._ICleanQueryState>.Empty;
      this.queryIds = Dafny.Sequence<BigInteger>.Empty;
      this.pendingState = CleanPendingRuntime.PendingState.Default();
    ...
    public Dafny.IMap<BigInteger,CleanLimitEngine._ICleanQueryState> queries {get; set;}
    public Dafny.ISequence<BigInteger> queryIds {get; set;}
    public CleanPendingRuntime._IPendingState pendingState {get; set;}
    ...
    public void __ctor()
    {
      ...
      (this).queries = Dafny.Map<BigInteger, CleanLimitEngine._ICleanQueryState>.FromElements();
      (this).queryIds = Dafny.Sequence<BigInteger>.FromElements();
      (this).pendingState = CleanPendingRuntime.__default.EmptyPendingState();
    ...
    public Dafny.ISequence<BigInteger> QueryVisible(BigInteger queryId)
    {
      ...
      if ((this.queries).Contains(queryId)) {
        ...
        _0_spec = (Dafny.Map<BigInteger, CleanLimitEngine._ICleanQueryState>.Select(this.queries,queryId)).dtor_spec;
        visible = DocsIndexTreap.__default.TreapCollectRange(this.docs, (_0_spec).dtor_minScore, (_0_spec).dtor_maxScore, (_0_spec).dtor_limit);
      } else {
        visible = Dafny.Sequence<BigInteger>.FromElements();
      }
      return visible;
    }
    public DocsIndexModel._IMaybeDocId QueryDocAtOffset(ThunderDbStack._IQuerySpec spec, BigInteger offset)
    {
      DocsIndexModel._IMaybeDocId doc = DocsIndexModel.MaybeDocId.Default();
      BigInteger _0_startRank;
      _0_startRank = DocsIndexTreap.__default.TreapRank(this.docs, (spec).dtor_minScore, DocsIndexModel.MaybeDocId.create_NoDoc());
      DocsIndexModel._IAtRank _source0 = DocsIndexTreap.__default.TreapGetAtRank(this.docs, (_0_startRank) + (offset));
      {
      ...
      }
      {
        ...
        {
          if ((_1_score) <= ((spec).dtor_maxScore)) {
            doc = DocsIndexModel.MaybeDocId.create_SomeDoc(_2_id);
          } else {
            doc = DocsIndexModel.MaybeDocId.create_NoDoc();
          }
        }
    ...
    public Dafny.ISequence<BigInteger> CollectVisibleQueriesInTreap(DocsIndexTreap._ITreap t, BigInteger id, BigInteger state)
    {
      Dafny.ISequence<BigInteger> matches = Dafny.Sequence<BigInteger>.Empty;
      matches = Dafny.Sequence<BigInteger>.FromElements();
      BigInteger _0_i;
      _0_i = BigInteger.Zero;
      while ((_0_i) < (new BigInteger((this.queryIds).Count))) {
        BigInteger _1_queryId;
        _1_queryId = (this.queryIds).Select(_0_i);
        if ((this.queries).Contains(_1_queryId)) {
          ThunderDbStack._IQuerySpec _2_spec;
          _2_spec = (Dafny.Map<BigInteger, CleanLimitEngine._ICleanQueryState>.Select(this.queries,_1_queryId)).dtor_spec;
          if (CleanLimitEngine.__default.DocVisibleInTreap(t, _2_spec, ThunderDbStack.__default.GetScore(state), id)) {
            matches = Dafny.Sequence<BigInteger>.Concat(matches, Dafny.Sequence<BigInteger>.FromElements(_1_queryId));
          }
        }
        _0_i = (_0_i) + (BigInteger.One);
      }
    ...
    public bool FillGap(BigInteger queryId)
    {
      bool registered = false;
      registered = false;
      if ((this.queries).Contains(queryId)) {
        CleanLimitEngine._ICleanQueryState _0_query;
        _0_query = Dafny.Map<BigInteger, CleanLimitEngine._ICleanQueryState>.Select(this.queries,queryId);
        if (((_0_query).dtor_currentMatches) < (((_0_query).dtor_spec).dtor_limit)) {
          BigInteger _1_offset;
          _1_offset = (_0_query).dtor_currentMatches;
          while (!(registered)) {
            DocsIndexModel._IMaybeDocId _2_candidate;
            DocsIndexModel._IMaybeDocId _out0;
            _out0 = (this).QueryDocAtOffset((_0_query).dtor_spec, _1_offset);
            _2_candidate = _out0;
            DocsIndexModel._IMaybeDocId _source0 = _2_candidate;
            {
              if (_source0.is_NoDoc) {
                {
                  return registered;
            ...
            }
            {
              ...
              {
                bool _4_alreadyPending;
                _4_alreadyPending = (((this.pendingState).dtor_byQuery).Contains(queryId)) && (ThunderDbStack.__default.ContainsId(Dafny.Map<BigInteger, Dafny.ISequence<BigInteger>>.Select((this.pendingState).dtor_byQuery,queryId), _3_docId));
                if (_4_alreadyPending) {
                  _1_offset = (_1_offset) + (BigInteger.One);
                } else {
                  (this).queries = Dafny.Map<BigInteger, CleanLimitEngine._ICleanQueryState>.Update(this.queries, queryId, CleanLimitEngine.CleanQueryState.create((_0_query).dtor_spec, ((_0_query).dtor_currentMatches) + (BigInteger.One)));
                  (this).pendingState = CleanPendingRuntime.__default.RegisterPending(this.pendingState, _3_docId, queryId);
                  registered = true;
                }
              }
            }
    ...
    public Dafny.ISequence<BigInteger> PickOverflowDoc(BigInteger queryId)
    {
      ...
      if ((this.queries).Contains(queryId)) {
        ...
        if (((_0_query).dtor_currentMatches) >= (((_0_query).dtor_spec).dtor_limit)) {
          ...
          DocsIndexModel._IMaybeDocId _out0;
          _out0 = (this).QueryDocAtOffset((_0_query).dtor_spec, ((_0_query).dtor_spec).dtor_limit);
          _1_candidate = _out0;
    ...
    public void AddQueryDeferred(ThunderDbStack._IQuerySpec spec, out BigInteger queryId, out Dafny.ISequence<ThunderDbStack._IDownstreamEvent> events)
    {
      ...
      (this).queryIds = Dafny.Sequence<BigInteger>.Concat(this.queryIds, Dafny.Sequence<BigInteger>.FromElements(queryId));
      Dafny.ISequence<BigInteger> _0_visible;
      _0_visible = DocsIndexTreap.__default.TreapCollectRange(this.docs, (spec).dtor_minScore, (spec).dtor_maxScore, (spec).dtor_limit);
      (this).queries = Dafny.Map<BigInteger, CleanLimitEngine._ICleanQueryState>.Update(this.queries, queryId, CleanLimitEngine.CleanQueryState.create(spec, new BigInteger((_0_visible).Count)));
      BigInteger _1_i;
      _1_i = BigInteger.Zero;
      while ((_1_i) < (new BigInteger((_0_visible).Count))) {
        (this).pendingState = CleanPendingRuntime.__default.RegisterPending(this.pendingState, (_0_visible).Select(_1_i), queryId);
        _1_i = (_1_i) + (BigInteger.One);
      }
      events = Dafny.Sequence<ThunderDbStack._IDownstreamEvent>.FromElements();
    }
    public Dafny.ISequence<ThunderDbStack._IDownstreamEvent> RemoveQueryDeferred(BigInteger queryId)
    {
      Dafny.ISequence<ThunderDbStack._IDownstreamEvent> events = Dafny.Sequence<ThunderDbStack._IDownstreamEvent>.Empty;
      if ((this.queries).Contains(queryId)) {
        (this).queries = Dafny.Helpers.Id<Func<BigInteger, Dafny.IMap<BigInteger,CleanLimitEngine._ICleanQueryState>>>((_0_queryId) => ((System.Func<Dafny.IMap<BigInteger,CleanLimitEngine._ICleanQueryState>>)(() => {
          var _coll0 = new System.Collections.Generic.List<Dafny.Pair<BigInteger,CleanLimitEngine._ICleanQueryState>>();
          foreach (BigInteger _compr_0 in (this.queries).Keys.Elements) {
            BigInteger _1_key = (BigInteger)_compr_0;
            if (((this.queries).Contains(_1_key)) && ((_1_key) != (_0_queryId))) {
              _coll0.Add(new Dafny.Pair<BigInteger,CleanLimitEngine._ICleanQueryState>(_1_key, Dafny.Map<BigInteger, CleanLimitEngine._ICleanQueryState>.Select(this.queries,_1_key)));
            }
          }
          return Dafny.Map<BigInteger,CleanLimitEngine._ICleanQueryState>.FromCollection(_coll0);
        }))())(queryId);
        (this).queryIds = CleanPendingRuntime.__default.RemoveQueryId(this.queryIds, queryId);
        if (((this.pendingState).dtor_byQuery).Contains(queryId)) {
          Dafny.ISequence<BigInteger> _2_pendingDocs;
          _2_pendingDocs = Dafny.Map<BigInteger, Dafny.ISequence<BigInteger>>.Select((this.pendingState).dtor_byQuery,queryId);
          BigInteger _3_i;
          _3_i = BigInteger.Zero;
          while ((_3_i) < (new BigInteger((_2_pendingDocs).Count))) {
            (this).pendingState = CleanPendingRuntime.__default.CancelPending(this.pendingState, (_2_pendingDocs).Select(_3_i), queryId);
            _3_i = (_3_i) + (BigInteger.One);
          }
        }
      }
    ...
    {
      Dafny.ISequence<ThunderDbStack._IDownstreamEvent> events = Dafny.Sequence<ThunderDbStack._IDownstreamEvent>.Empty;
      Dafny.ISequence<ThunderDbStack._IRetrievalDoc> _0_docs;
      _0_docs = CleanPendingRuntime.__default.BuildGroupedRetrievals((this.pendingState).dtor_docOrder, (this.pendingState).dtor_byDoc, this.store);
      (this).pendingState = CleanPendingRuntime.__default.EmptyPendingState();
      if ((new BigInteger((_0_docs).Count)).Sign == 0) {
        events = Dafny.Sequence<ThunderDbStack._IDownstreamEvent>.FromElements();
      } else {
        events = Dafny.Sequence<ThunderDbStack._IDownstreamEvent>.FromElements(ThunderDbStack.DownstreamEvent.create_RetrievalEvent(_0_docs));
      }
      return events;
    }
    public Dafny.ISequence<ThunderDbStack._IDownstreamEvent> ApplyDocChangeDeferred(BigInteger id, ThunderDbStack._IMaybeDocState oldState, ThunderDbStack._IMaybeDocState newState)
    {
      ...
      if ((((oldState).is_HasState) && ((newState).is_HasState)) && ((ThunderDbStack.__default.GetScore((oldState).dtor_state)) == (ThunderDbStack.__default.GetScore((newState).dtor_state)))) {
        ...
        _out0 = (this).CollectVisibleQueriesInTreap(this.docs, id, (newState).dtor_state);
        _0_covering = _out0;
        (this).pendingState = CleanPendingRuntime.__default.ResolvePendingForDoc(this.pendingState, id);
      ...
      DocsIndexTreap._ITreap _1_docsBefore;
      _1_docsBefore = this.docs;
      Dafny.ISequence<BigInteger> _2_oldMatches;
      _2_oldMatches = Dafny.Sequence<BigInteger>.FromElements();
      Dafny.ISequence<BigInteger> _3_newMatches;
      _3_newMatches = Dafny.Sequence<BigInteger>.FromElements();
      Dafny.ISequence<BigInteger> _4_blockedCandidates;
      _4_blockedCandidates = Dafny.Sequence<BigInteger>.FromElements();
      if ((oldState).is_HasState) {
        Dafny.ISequence<BigInteger> _out1;
        _out1 = (this).CollectVisibleQueriesInTreap(_1_docsBefore, id, (oldState).dtor_state);
        _2_oldMatches = _out1;
      ...
      if ((newState).is_HasState) {
        ...
        _out2 = (this).CollectVisibleQueriesInTreap(this.docs, id, (newState).dtor_state);
        _3_newMatches = _out2;
      } else {
      ...
      Dafny.IMap<BigInteger,bool> _5_oldMatchMap;
      _5_oldMatchMap = Dafny.Map<BigInteger, bool>.FromElements();
      Dafny.IMap<BigInteger,bool> _6_newMatchMap;
      _6_newMatchMap = Dafny.Map<BigInteger, bool>.FromElements();
      BigInteger _7_i;
      _7_i = BigInteger.Zero;
      while ((_7_i) < (new BigInteger((_2_oldMatches).Count))) {
        _5_oldMatchMap = Dafny.Map<BigInteger, bool>.Update(_5_oldMatchMap, (_2_oldMatches).Select(_7_i), true);
        _7_i = (_7_i) + (BigInteger.One);
      }
      _7_i = BigInteger.Zero;
      while ((_7_i) < (new BigInteger((_3_newMatches).Count))) {
        _6_newMatchMap = Dafny.Map<BigInteger, bool>.Update(_6_newMatchMap, (_3_newMatches).Select(_7_i), true);
        _7_i = (_7_i) + (BigInteger.One);
      }
      _7_i = BigInteger.Zero;
      while ((_7_i) < (new BigInteger((_2_oldMatches).Count))) {
        if ((this.queries).Contains((_2_oldMatches).Select(_7_i))) {
          CleanLimitEngine._ICleanQueryState _8_query;
          _8_query = Dafny.Map<BigInteger, CleanLimitEngine._ICleanQueryState>.Select(this.queries,(_2_oldMatches).Select(_7_i));
          if (((_8_query).dtor_currentMatches).Sign == 1) {
            (this).queries = Dafny.Map<BigInteger, CleanLimitEngine._ICleanQueryState>.Update(this.queries, (_2_oldMatches).Select(_7_i), CleanLimitEngine.CleanQueryState.create((_8_query).dtor_spec, ((_8_query).dtor_currentMatches) - (BigInteger.One)));
          }
        }
        _7_i = (_7_i) + (BigInteger.One);
      }
      _7_i = BigInteger.Zero;
      while ((_7_i) < (new BigInteger((_3_newMatches).Count))) {
        if ((this.queries).Contains((_3_newMatches).Select(_7_i))) {
          CleanLimitEngine._ICleanQueryState _9_query;
          _9_query = Dafny.Map<BigInteger, CleanLimitEngine._ICleanQueryState>.Select(this.queries,(_3_newMatches).Select(_7_i));
          bool _10_alreadyPending;
          _10_alreadyPending = (((this.pendingState).dtor_byDoc).Contains(id)) && (ThunderDbStack.__default.ContainsQueryId(Dafny.Map<BigInteger, Dafny.ISequence<BigInteger>>.Select((this.pendingState).dtor_byDoc,id), (_3_newMatches).Select(_7_i)));
          if (_10_alreadyPending) {
          } else if (((_9_query).dtor_currentMatches) < (((_9_query).dtor_spec).dtor_limit)) {
            (this).queries = Dafny.Map<BigInteger, CleanLimitEngine._ICleanQueryState>.Update(this.queries, (_3_newMatches).Select(_7_i), CleanLimitEngine.CleanQueryState.create((_9_query).dtor_spec, ((_9_query).dtor_currentMatches) + (BigInteger.One)));
          } else {
            _4_blockedCandidates = Dafny.Sequence<BigInteger>.Concat(_4_blockedCandidates, Dafny.Sequence<BigInteger>.FromElements((_3_newMatches).Select(_7_i)));
          }
        }
        _7_i = (_7_i) + (BigInteger.One);
      }
      Dafny.ISequence<ThunderDbStack._IEviction> _11_evictions;
      _11_evictions = Dafny.Sequence<ThunderDbStack._IEviction>.FromElements();
      _7_i = BigInteger.Zero;
      while ((_7_i) < (new BigInteger((_2_oldMatches).Count))) {
        if (!((_6_newMatchMap).Contains((_2_oldMatches).Select(_7_i)))) {
          bool _12_gapRegistered;
          bool _out3;
          _out3 = (this).FillGap((_2_oldMatches).Select(_7_i));
          _12_gapRegistered = _out3;
        }
        _7_i = (_7_i) + (BigInteger.One);
      }
      _7_i = BigInteger.Zero;
      while ((_7_i) < (new BigInteger((_4_blockedCandidates).Count))) {
        if (!((_5_oldMatchMap).Contains((_4_blockedCandidates).Select(_7_i)))) {
          Dafny.ISequence<BigInteger> _13_evicted;
          Dafny.ISequence<BigInteger> _out4;
          _out4 = (this).PickOverflowDoc((_4_blockedCandidates).Select(_7_i));
          _13_evicted = _out4;
          if ((new BigInteger((_13_evicted).Count)).Sign == 1) {
            _11_evictions = Dafny.Sequence<ThunderDbStack._IEviction>.Concat(_11_evictions, Dafny.Sequence<ThunderDbStack._IEviction>.FromElements(ThunderDbStack.Eviction.create((_4_blockedCandidates).Select(_7_i), (_13_evicted).Select(BigInteger.Zero))));
            bool _14_cancelled;
            _14_cancelled = (((this.pendingState).dtor_byDoc).Contains((_13_evicted).Select(BigInteger.Zero))) && (ThunderDbStack.__default.ContainsQueryId(Dafny.Map<BigInteger, Dafny.ISequence<BigInteger>>.Select((this.pendingState).dtor_byDoc,(_13_evicted).Select(BigInteger.Zero)), (_4_blockedCandidates).Select(_7_i)));
            (this).pendingState = CleanPendingRuntime.__default.CancelPending(this.pendingState, (_13_evicted).Select(BigInteger.Zero), (_4_blockedCandidates).Select(_7_i));
            if (_14_cancelled) {
              bool _15_replacement;
              bool _out5;
              _out5 = (this).FillGap((_4_blockedCandidates).Select(_7_i));
              _15_replacement = _out5;
            }
          }
        }
        _7_i = (_7_i) + (BigInteger.One);
      ...
      (this).pendingState = CleanPendingRuntime.__default.ResolvePendingForDoc(this.pendingState, id);
      ThunderDbStack._IMatchPayload _16_payload;
      _16_payload = ThunderDbStack.MatchPayload.create(id, oldState, newState, _2_oldMatches, _3_newMatches, _11_evictions);
      if (ThunderDbStack.__default.HasAnyMatchChange(_16_payload)) {
        events = Dafny.Sequence<ThunderDbStack._IDownstreamEvent>.FromElements(ThunderDbStack.DownstreamEvent.create_MatchEvent(_16_payload));
      }
      return events;
    }
    ...
    public void ProcessItem(ThunderDbStack._IStreamItem item, out Dafny.ISequence<ThunderDbStack._IDownstreamEvent> events, out BigInteger queryId)
    {
      ...
      Dafny.ISequence<ThunderDbStack._IDownstreamEvent> _out2;
      _out2 = (this).DrainPendingRetrievalsGrouped();
...
namespace CleanLimitParity {
  ...
  public partial class __default {
    ...
    {
      CleanLimitParity.CleanParityRunner _0_runner;
      CleanLimitParity.CleanParityRunner _nw0 = new CleanLimitParity.CleanParityRunner();
      _nw0.__ctor();
      _0_runner = _nw0;
      (_0_runner).RunScenario(Dafny.Sequence<Dafny.Rune>.UnicodeFromString("clean-small"), BigInteger.One, new BigInteger(8), new BigInteger(4), new BigInteger(4), new BigInteger(2), BigInteger.One, BigInteger.One, new BigInteger(3), new BigInteger(2));
      CleanLimitParity.CleanParityRunner _1_runner2;
      CleanLimitParity.CleanParityRunner _nw1 = new CleanLimitParity.CleanParityRunner();
      _nw1.__ctor();
      _1_runner2 = _nw1;
      (_1_runner2).RunScenario(Dafny.Sequence<Dafny.Rune>.UnicodeFromString("clean-medium"), new BigInteger(17), new BigInteger(20), new BigInteger(8), new BigInteger(8), new BigInteger(3), new BigInteger(2), new BigInteger(2), new BigInteger(5), new BigInteger(3));
    }
  ...
  public partial class CleanParityRunner {
    public CleanParityRunner() {
      this.engine = default(CleanLimitEngine.CleanEngine);
    }
    public CleanLimitEngine.CleanEngine engine {get; set;}
    public void __ctor()
    {
    ...
    public void CheckState(Dafny.ISequence<Dafny.Rune> name, BigInteger opIndex, ThunderDbStack._IStreamItem item, ThunderDbStack._IEngineState state, Dafny.ISequence<ThunderDbStack._IDownstreamEvent> functionalEvents, Dafny.ISequence<ThunderDbStack._IDownstreamEvent> cleanEvents, BigInteger functionalQueryId, BigInteger cleanQueryId)
    {
      bool _0_mismatch;
      _0_mismatch = (((((((functionalQueryId) != (cleanQueryId)) || (!(CleanLimitParity.__default.NormalizeEvents(functionalEvents)).Equals(CleanLimitParity.__default.NormalizeEvents(cleanEvents)))) || (!((state).dtor_store).Equals(this.engine.store))) || (!((state).dtor_docIds).Equals(this.engine.docIds))) || (!(DocsIndexTreap.__default.Entries((state).dtor_treap)).Equals(DocsIndexTreap.__default.Entries(this.engine.docs)))) || (((state).dtor_nextQueryId) != (this.engine.nextQueryId))) || ((new BigInteger(((state).dtor_queries).Count)) != (new BigInteger((this.engine.queries).Count)));
      if (_0_mismatch) {
        ...
        Dafny.Helpers.Print((this.engine.store));
        ...
        Dafny.Helpers.Print((DocsIndexTreap.__default.Entries(this.engine.docs)));
      ...
      while ((_1_i) < (new BigInteger(((state).dtor_queries).Count))) {
        ...
        Dafny.ISequence<BigInteger> _out0;
        _out0 = (this.engine).QueryVisible((_2_query).dtor_id);
        _3_cleanVisible = _out0;
        if (((!((this.engine.queries).Contains((_2_query).dtor_id))) || (!object.Equals((Dafny.Map<BigInteger, CleanLimitEngine._ICleanQueryState>.Select(this.engine.queries,(_2_query).dtor_id)).dtor_spec, (_2_query).dtor_spec))) || (!(_3_cleanVisible).Equals((_2_query).dtor_visible))) {
          Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString("scenario ")).ToVerbatimString(false));
          Dafny.Helpers.Print((name).ToVerbatimString(false));
          ...
          if ((this.engine.queries).Contains((_2_query).dtor_id)) {
            Dafny.Helpers.Print(((Dafny.Map<BigInteger, CleanLimitEngine._ICleanQueryState>.Select(this.engine.queries,(_2_query).dtor_id)).dtor_spec));
          } else {
      ...
      if (_0_mismatch) {
        if (!(false)) {
          throw new Dafny.HaltException("specs/clean-dafny/CleanLimitParity.dfy(144,8): " + Dafny.Sequence<Dafny.Rune>.UnicodeFromString("expectation violation").ToVerbatimString(false));}
      }
    }
    public void RunScenario(Dafny.ISequence<Dafny.Rune> name, BigInteger seed, BigInteger documents, BigInteger customers, BigInteger ticks, BigInteger updatesPerTick, BigInteger insertsPerTick, BigInteger deletesPerTick, BigInteger queryLimit, BigInteger density)
    {
      ...
      BigInteger _6_opIndex;
      _6_opIndex = BigInteger.Zero;
      BigInteger _7_i;
      _7_i = BigInteger.Zero;
      while ((_7_i) < (documents)) {
        ...
        BigInteger _out0;
        BigInteger _out1;
        CleanLimitParity.__default.NextRand(_2_rng, out _out0, out _out1);
        _8_nextRng = _out0;
        _9_scoreSeed = _out1;
        _2_rng = _8_nextRng;
        _3_seedDocs = Dafny.Sequence<ThunderDbStack._ISeedDoc>.Concat(_3_seedDocs, Dafny.Sequence<ThunderDbStack._ISeedDoc>.FromElements(ThunderDbStack.SeedDoc.create(_7_i, Dafny.Helpers.EuclideanModulus(_9_scoreSeed, _5_range))));
        _7_i = (_7_i) + (BigInteger.One);
      }
      ThunderDbStack._IStreamItem _10_seedItem;
      _10_seedItem = ThunderDbStack.StreamItem.create_SeedDocsItem(_3_seedDocs);
      ThunderDbStack._IEngineState _11_nextState;
      Dafny.ISequence<ThunderDbStack._IDownstreamEvent> _12_functionalEvents;
      BigInteger _13_functionalQueryId;
      ThunderDbStack._IEngineState _out2;
      Dafny.ISequence<ThunderDbStack._IDownstreamEvent> _out3;
      BigInteger _out4;
      ThunderDbStack.__default.ProcessItem(_0_state, _10_seedItem, out _out2, out _out3, out _out4);
      _11_nextState = _out2;
      _12_functionalEvents = _out3;
      _13_functionalQueryId = _out4;
      Dafny.ISequence<ThunderDbStack._IDownstreamEvent> _14_cleanEvents;
      BigInteger _15_cleanQueryId;
      Dafny.ISequence<ThunderDbStack._IDownstreamEvent> _out5;
      BigInteger _out6;
      (this.engine).ProcessItem(_10_seedItem, out _out5, out _out6);
      _14_cleanEvents = _out5;
      _15_cleanQueryId = _out6;
      _0_state = _11_nextState;
      _1_summary = ThunderDbStack.__default.UpdateSummary(_1_summary, _12_functionalEvents);
      (this).CheckState(name, _6_opIndex, _10_seedItem, _0_state, _12_functionalEvents, _14_cleanEvents, _13_functionalQueryId, _15_cleanQueryId);
      _7_i = BigInteger.Zero;
      while ((_7_i) < (customers)) {
        BigInteger _16_nextRng1;
        BigInteger _17_widthSeed;
        BigInteger _out7;
        BigInteger _out8;
        CleanLimitParity.__default.NextRand(_2_rng, out _out7, out _out8);
        _16_nextRng1 = _out7;
        _17_widthSeed = _out8;
        _2_rng = _16_nextRng1;
        BigInteger _18_nextRng2;
        BigInteger _19_minSeed;
        BigInteger _out9;
        BigInteger _out10;
        CleanLimitParity.__default.NextRand(_2_rng, out _out9, out _out10);
        _18_nextRng2 = _out9;
        _19_minSeed = _out10;
        _2_rng = _18_nextRng2;
        BigInteger _20_width;
        _20_width = (BigInteger.One) + (Dafny.Helpers.EuclideanModulus(_17_widthSeed, (Dafny.Helpers.EuclideanDivision(_5_range, new BigInteger(2))) + (BigInteger.One)));
        BigInteger _21_minScore;
        _21_minScore = Dafny.Helpers.EuclideanModulus(_19_minSeed, _5_range);
        BigInteger _22_maxScore;
        _22_maxScore = (_21_minScore) + (_20_width);
        ThunderDbStack._IStreamItem _23_item;
        _23_item = ThunderDbStack.StreamItem.create_QueryAddItem(ThunderDbStack.QuerySpec.create(_21_minScore, _22_maxScore, queryLimit));
        ThunderDbStack._IEngineState _24_stateAfterAdd;
        Dafny.ISequence<ThunderDbStack._IDownstreamEvent> _25_functionalAddEvents;
        BigInteger _26_functionalAddQueryId;
        ThunderDbStack._IEngineState _out11;
        Dafny.ISequence<ThunderDbStack._IDownstreamEvent> _out12;
        BigInteger _out13;
        ThunderDbStack.__default.ProcessItem(_0_state, _23_item, out _out11, out _out12, out _out13);
        _24_stateAfterAdd = _out11;
        _25_functionalAddEvents = _out12;
        _26_functionalAddQueryId = _out13;
        Dafny.ISequence<ThunderDbStack._IDownstreamEvent> _27_cleanAddEvents;
        BigInteger _28_cleanAddQueryId;
        Dafny.ISequence<ThunderDbStack._IDownstreamEvent> _out14;
        BigInteger _out15;
        (this.engine).ProcessItem(_23_item, out _out14, out _out15);
        _27_cleanAddEvents = _out14;
        _28_cleanAddQueryId = _out15;
        _0_state = _24_stateAfterAdd;
        _1_summary = ThunderDbStack.__default.UpdateSummary(_1_summary, _25_functionalAddEvents);
        _6_opIndex = (_6_opIndex) + (BigInteger.One);
        (this).CheckState(name, _6_opIndex, _23_item, _0_state, _25_functionalAddEvents, _27_cleanAddEvents, _26_functionalAddQueryId, _28_cleanAddQueryId);
        _7_i = (_7_i) + (BigInteger.One);
      }
      BigInteger _29_tick;
      _29_tick = BigInteger.Zero;
      while ((_29_tick) < (ticks)) {
        BigInteger _30_update;
        _30_update = BigInteger.Zero;
        while ((_30_update) < (updatesPerTick)) {
          if ((new BigInteger(((_0_state).dtor_docIds).Count)).Sign == 1) {
            BigInteger _31_nextRng3;
            BigInteger _32_pickSeed;
            BigInteger _out16;
            BigInteger _out17;
            CleanLimitParity.__default.NextRand(_2_rng, out _out16, out _out17);
            _31_nextRng3 = _out16;
            _32_pickSeed = _out17;
            _2_rng = _31_nextRng3;
            BigInteger _33_nextRng4;
            BigInteger _34_scoreSeed;
            BigInteger _out18;
            BigInteger _out19;
            CleanLimitParity.__default.NextRand(_2_rng, out _out18, out _out19);
            _33_nextRng4 = _out18;
            _34_scoreSeed = _out19;
            _2_rng = _33_nextRng4;
            BigInteger _35_picked;
            _35_picked = Dafny.Helpers.EuclideanModulus(_32_pickSeed, new BigInteger(((_0_state).dtor_docIds).Count));
            BigInteger _36_docId;
            _36_docId = ((_0_state).dtor_docIds).Select(_35_picked);
            ThunderDbStack._IMaybeDocState _source0 = ThunderDbStack.__default.LookupState((_0_state).dtor_store, _36_docId);
            {
              if (_source0.is_NoState) {
                goto after_match0;
              }
            ...
            {
              BigInteger _37_docState = _source0.dtor_state;
              {
                ThunderDbStack._IStreamItem _38_item;
                _38_item = ThunderDbStack.StreamItem.create_DocChangeItem(_36_docId, ThunderDbStack.MaybeDocState.create_HasState(_37_docState), ThunderDbStack.MaybeDocState.create_HasState(Dafny.Helpers.EuclideanModulus(_34_scoreSeed, _5_range)));
                ThunderDbStack._IEngineState _39_stateAfterUpdate;
                Dafny.ISequence<ThunderDbStack._IDownstreamEvent> _40_functionalUpdateEvents;
                BigInteger _41_functionalUpdateQueryId;
                ThunderDbStack._IEngineState _out20;
                Dafny.ISequence<ThunderDbStack._IDownstreamEvent> _out21;
                BigInteger _out22;
                ThunderDbStack.__default.ProcessItem(_0_state, _38_item, out _out20, out _out21, out _out22);
                _39_stateAfterUpdate = _out20;
                _40_functionalUpdateEvents = _out21;
                _41_functionalUpdateQueryId = _out22;
                Dafny.ISequence<ThunderDbStack._IDownstreamEvent> _42_cleanUpdateEvents;
                BigInteger _43_cleanUpdateQueryId;
                Dafny.ISequence<ThunderDbStack._IDownstreamEvent> _out23;
                BigInteger _out24;
                (this.engine).ProcessItem(_38_item, out _out23, out _out24);
                _42_cleanUpdateEvents = _out23;
                _43_cleanUpdateQueryId = _out24;
                _0_state = _39_stateAfterUpdate;
                _1_summary = ThunderDbStack.__default.UpdateSummary(_1_summary, _40_functionalUpdateEvents);
                _6_opIndex = (_6_opIndex) + (BigInteger.One);
                (this).CheckState(name, _6_opIndex, _38_item, _0_state, _40_functionalUpdateEvents, _42_cleanUpdateEvents, _41_functionalUpdateQueryId, _43_cleanUpdateQueryId);
              }
            }
          after_match0: ;
          }
          _30_update = (_30_update) + (BigInteger.One);
        }
        BigInteger _44_insert;
        _44_insert = BigInteger.Zero;
        while ((_44_insert) < (insertsPerTick)) {
          BigInteger _45_nextRng5;
          BigInteger _46_scoreSeed;
          BigInteger _out25;
          BigInteger _out26;
          CleanLimitParity.__default.NextRand(_2_rng, out _out25, out _out26);
          _45_nextRng5 = _out25;
          _46_scoreSeed = _out26;
          _2_rng = _45_nextRng5;
          ThunderDbStack._IStreamItem _47_item;
          _47_item = ThunderDbStack.StreamItem.create_DocChangeItem(_4_nextDocId, ThunderDbStack.MaybeDocState.create_NoState(), ThunderDbStack.MaybeDocState.create_HasState(Dafny.Helpers.EuclideanModulus(_46_scoreSeed, _5_range)));
          ThunderDbStack._IEngineState _48_stateAfterInsert;
          Dafny.ISequence<ThunderDbStack._IDownstreamEvent> _49_functionalInsertEvents;
          BigInteger _50_functionalInsertQueryId;
          ThunderDbStack._IEngineState _out27;
          Dafny.ISequence<ThunderDbStack._IDownstreamEvent> _out28;
          BigInteger _out29;
          ThunderDbStack.__default.ProcessItem(_0_state, _47_item, out _out27, out _out28, out _out29);
          _48_stateAfterInsert = _out27;
          _49_functionalInsertEvents = _out28;
          _50_functionalInsertQueryId = _out29;
          Dafny.ISequence<ThunderDbStack._IDownstreamEvent> _51_cleanInsertEvents;
          BigInteger _52_cleanInsertQueryId;
          Dafny.ISequence<ThunderDbStack._IDownstreamEvent> _out30;
          BigInteger _out31;
          (this.engine).ProcessItem(_47_item, out _out30, out _out31);
          _51_cleanInsertEvents = _out30;
          _52_cleanInsertQueryId = _out31;
          _0_state = _48_stateAfterInsert;
          _1_summary = ThunderDbStack.__default.UpdateSummary(_1_summary, _49_functionalInsertEvents);
          _4_nextDocId = (_4_nextDocId) + (BigInteger.One);
          _6_opIndex = (_6_opIndex) + (BigInteger.One);
          (this).CheckState(name, _6_opIndex, _47_item, _0_state, _49_functionalInsertEvents, _51_cleanInsertEvents, _50_functionalInsertQueryId, _52_cleanInsertQueryId);
          _44_insert = (_44_insert) + (BigInteger.One);
        }
        BigInteger _53_delete;
        _53_delete = BigInteger.Zero;
        while ((_53_delete) < (deletesPerTick)) {
          if ((new BigInteger(((_0_state).dtor_docIds).Count)).Sign == 1) {
            BigInteger _54_nextRng6;
            BigInteger _55_pickSeed;
            BigInteger _out32;
            BigInteger _out33;
            CleanLimitParity.__default.NextRand(_2_rng, out _out32, out _out33);
            _54_nextRng6 = _out32;
            _55_pickSeed = _out33;
            _2_rng = _54_nextRng6;
            BigInteger _56_picked;
            _56_picked = Dafny.Helpers.EuclideanModulus(_55_pickSeed, new BigInteger(((_0_state).dtor_docIds).Count));
            BigInteger _57_docId;
            _57_docId = ((_0_state).dtor_docIds).Select(_56_picked);
            ThunderDbStack._IMaybeDocState _source1 = ThunderDbStack.__default.LookupState((_0_state).dtor_store, _57_docId);
            {
            ...
            {
              BigInteger _58_docState = _source1.dtor_state;
              {
                ThunderDbStack._IStreamItem _59_item;
                _59_item = ThunderDbStack.StreamItem.create_DocChangeItem(_57_docId, ThunderDbStack.MaybeDocState.create_HasState(_58_docState), ThunderDbStack.MaybeDocState.create_NoState());
                ThunderDbStack._IEngineState _60_stateAfterDelete;
                Dafny.ISequence<ThunderDbStack._IDownstreamEvent> _61_functionalDeleteEvents;
                BigInteger _62_functionalDeleteQueryId;
                ThunderDbStack._IEngineState _out34;
                Dafny.ISequence<ThunderDbStack._IDownstreamEvent> _out35;
                BigInteger _out36;
                ThunderDbStack.__default.ProcessItem(_0_state, _59_item, out _out34, out _out35, out _out36);
                _60_stateAfterDelete = _out34;
                _61_functionalDeleteEvents = _out35;
                _62_functionalDeleteQueryId = _out36;
                Dafny.ISequence<ThunderDbStack._IDownstreamEvent> _63_cleanDeleteEvents;
                BigInteger _64_cleanDeleteQueryId;
                Dafny.ISequence<ThunderDbStack._IDownstreamEvent> _out37;
                BigInteger _out38;
                (this.engine).ProcessItem(_59_item, out _out37, out _out38);
                _63_cleanDeleteEvents = _out37;
                _64_cleanDeleteQueryId = _out38;
                _0_state = _60_stateAfterDelete;
                _1_summary = ThunderDbStack.__default.UpdateSummary(_1_summary, _61_functionalDeleteEvents);
                _6_opIndex = (_6_opIndex) + (BigInteger.One);
                (this).CheckState(name, _6_opIndex, _59_item, _0_state, _61_functionalDeleteEvents, _63_cleanDeleteEvents, _62_functionalDeleteQueryId, _64_cleanDeleteQueryId);
              }
            }
          after_match1: ;
          }
          _53_delete = (_53_delete) + (BigInteger.One);
        }
        _29_tick = (_29_tick) + (BigInteger.One);
      }
      Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString("scenario ")).ToVerbatimString(false));
      Dafny.Helpers.Print((name).ToVerbatimString(false));
