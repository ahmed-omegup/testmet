...
// Command-line arguments: run specs/dafny/ThunderDbStackPerf.dfy --no-verify
...
module ThunderDbStack {
  ...
  function UpdateQueriesForDocChange(queries: seq<QueryRuntime>, treap: Treap, store: seq<StoredDoc>, id: DocId, oldState: MaybeDocState, newState: MaybeDocState): seq<QueryRuntime>
    requires SumConsistent(treap)
    ...
    decreases state
  {
    ...
    state.entries == Entries(state.treap) &&
  ...
  function BuildGapRetrievalForQuery(oldQuery: QueryRuntime, newQuery: QueryRuntime, store: seq<StoredDoc>, changedId: DocId): seq<RetrievalDoc>
    ...
    decreases state, docs
  {
    ...
    next := EngineState(store, entries, treap, RecomputeQueries(state.queries, treap), state.nextQueryId);
  }
    ...
    decreases state, spec
  {
    ...
    next := EngineState(state.store, state.entries, state.treap, state.queries + [query], state.nextQueryId + 1);
  ...
  method RemoveQuery(state: EngineState, id: QueryId)
    ...
    decreases state, id
  {
    next := EngineState(state.store, state.entries, state.treap, RemoveQueryRuntime(state.queries, id), state.nextQueryId);
    events := [];
  }
    ...
    decreases state, id, oldState, newState
  {
    ...
    case {:split false} HasState(oldDoc) =>
      ...
      case {:split false} HasState(newDoc) =>
        ...
        next := EngineState(store, entries, treap, newQueries, state.nextQueryId);
  ...
  datatype EngineState = EngineState(store: seq<StoredDoc>, entries: seq<Entry>, treap: Treap, queries: seq<QueryRuntime>, nextQueryId: QueryId)
}
...
namespace ThunderDbStack {
  ...
  public partial class __default {
    ...
    public static Dafny.ISequence<ThunderDbStack._IQueryRuntime> UpdateQueriesForDocChange(Dafny.ISequence<ThunderDbStack._IQueryRuntime> queries, DocsIndexTreap._ITreap treap, Dafny.ISequence<ThunderDbStack._IStoredDoc> store, BigInteger id, ThunderDbStack._IMaybeDocState oldState, ThunderDbStack._IMaybeDocState newState)
    {
      Dafny.ISequence<ThunderDbStack._IQueryRuntime> _0___accumulator = Dafny.Sequence<ThunderDbStack._IQueryRuntime>.FromElements();
    TAIL_CALL_START: ;
      if ((new BigInteger((queries).Count)).Sign == 0) {
    ...
    public static bool ValidState(ThunderDbStack._IEngineState state) {
      return ((((ThunderDbStack.__default.UniqueStoreIds((state).dtor_store)) && (DocsIndexTreap.__default.SumConsistent((state).dtor_treap))) && (((state).dtor_entries).Equals(ThunderDbStack.__default.BuildEntriesFromStore((state).dtor_store)))) && (((state).dtor_entries).Equals(DocsIndexTreap.__default.Entries((state).dtor_treap)))) && (ThunderDbStack.__default.QuerysConsistent((state).dtor_queries, (state).dtor_entries));
    }
    public static ThunderDbStack._IEngineState EmptyState() {
      return ThunderDbStack.EngineState.create(Dafny.Sequence<ThunderDbStack._IStoredDoc>.FromElements(), Dafny.Sequence<DocsIndexModel._IEntry>.FromElements(), DocsIndexTreap.Treap.create_Empty(), Dafny.Sequence<ThunderDbStack._IQueryRuntime>.FromElements(), BigInteger.One);
    }
    ...
    public static Dafny.ISequence<ThunderDbStack._IRetrievalDoc> BuildGapRetrievalForQuery(ThunderDbStack._IQueryRuntime oldQuery, ThunderDbStack._IQueryRuntime newQuery, Dafny.ISequence<ThunderDbStack._IStoredDoc> store, BigInteger changedId)
    {
    ...
    public static ThunderDbStack._IEngineState SeedDocs(ThunderDbStack._IEngineState state, Dafny.ISequence<ThunderDbStack._ISeedDoc> docs)
    {
      ...
      DocsIndexTreap._ITreap _1_treap;
      _1_treap = (state).dtor_treap;
      BigInteger _2_i;
      _2_i = BigInteger.Zero;
      while ((_2_i) < (new BigInteger((docs).Count))) {
        _0_store = ThunderDbStack.__default.PutStoredDoc(_0_store, ((docs).Select(_2_i)).dtor_id, ((docs).Select(_2_i)).dtor_state);
        _1_treap = DocsIndexTreap.__default.Add(_1_treap, (((docs).Select(_2_i)).dtor_state), ((docs).Select(_2_i)).dtor_id, ThunderDbStack.__default.PriorityFor((((docs).Select(_2_i)).dtor_state), ((docs).Select(_2_i)).dtor_id));
        _2_i = (_2_i) + (BigInteger.One);
      }
      Dafny.ISequence<DocsIndexModel._IEntry> _3_entries;
      _3_entries = ThunderDbStack.__default.BuildEntriesFromStore(_0_store);
      next = ThunderDbStack.EngineState.create(_0_store, _3_entries, _1_treap, ThunderDbStack.__default.RecomputeQueries((state).dtor_queries, _1_treap), (state).dtor_nextQueryId);
      return next;
    }
    public static void AddQuery(ThunderDbStack._IEngineState state, ThunderDbStack._IQuerySpec spec, out ThunderDbStack._IEngineState next, out Dafny.ISequence<ThunderDbStack._IDownstreamEvent> events, out BigInteger queryId)
    {
      ...
      next = ThunderDbStack.EngineState.create((state).dtor_store, (state).dtor_entries, (state).dtor_treap, Dafny.Sequence<ThunderDbStack._IQueryRuntime>.Concat((state).dtor_queries, Dafny.Sequence<ThunderDbStack._IQueryRuntime>.FromElements(_1_query)), ((state).dtor_nextQueryId) + (BigInteger.One));
    ...
    public static void RemoveQuery(ThunderDbStack._IEngineState state, BigInteger id, out ThunderDbStack._IEngineState next, out Dafny.ISequence<ThunderDbStack._IDownstreamEvent> events)
    {
      next = ThunderDbStack.EngineState.Default();
      events = Dafny.Sequence<ThunderDbStack._IDownstreamEvent>.Empty;
      next = ThunderDbStack.EngineState.create((state).dtor_store, (state).dtor_entries, (state).dtor_treap, ThunderDbStack.__default.RemoveQueryRuntime((state).dtor_queries, id), (state).dtor_nextQueryId);
      events = Dafny.Sequence<ThunderDbStack._IDownstreamEvent>.FromElements();
    }
    public static void ApplyDocChange(ThunderDbStack._IEngineState state, BigInteger id, ThunderDbStack._IMaybeDocState oldState, ThunderDbStack._IMaybeDocState newState, out ThunderDbStack._IEngineState next, out Dafny.ISequence<ThunderDbStack._IDownstreamEvent> events)
    {
      ...
      {
        BigInteger _3_oldDoc = _source0.dtor_state;
        {
          _0_store = ThunderDbStack.__default.RemoveStoredDoc(_0_store, id);
          _1_entries = ThunderDbStack.__default.RefRemoveEntry(_1_entries, ThunderDbStack.__default.GetScore(_3_oldDoc), id);
        ...
        {
          BigInteger _4_newDoc = _source1.dtor_state;
          {
            _0_store = ThunderDbStack.__default.PutStoredDoc(_0_store, id, _4_newDoc);
            _1_entries = ThunderDbStack.__default.RefInsertEntry(_1_entries, ThunderDbStack.__default.GetScore(_4_newDoc), id);
          ...
          next = ThunderDbStack.EngineState.create(_0_store, _1_entries, _2_treap, _5_newQueries, (state).dtor_nextQueryId);
  ...
  public class EngineState : _IEngineState {
    ...
    public EngineState(Dafny.ISequence<ThunderDbStack._IStoredDoc> store, Dafny.ISequence<DocsIndexModel._IEntry> entries, DocsIndexTreap._ITreap treap, Dafny.ISequence<ThunderDbStack._IQueryRuntime> queries, BigInteger nextQueryId) {
      this._store = store;
    ...
    public _IEngineState DowncastClone() {
      ...
      return new EngineState(_store, _entries, _treap, _queries, _nextQueryId);
    }
    public override bool Equals(object other) {
      var oth = other as ThunderDbStack.EngineState;
      return oth != null && object.Equals(this._store, oth._store) && object.Equals(this._entries, oth._entries) && object.Equals(this._treap, oth._treap) && object.Equals(this._queries, oth._queries) && this._nextQueryId == oth._nextQueryId;
    }
    public override int GetHashCode() {
      ulong hash = 5381;
      hash = ((hash << 5) + hash) + 0;
    ...
    private static readonly ThunderDbStack._IEngineState theDefault = create(Dafny.Sequence<ThunderDbStack._IStoredDoc>.Empty, Dafny.Sequence<DocsIndexModel._IEntry>.Empty, DocsIndexTreap.Treap.Default(), Dafny.Sequence<ThunderDbStack._IQueryRuntime>.Empty, BigInteger.Zero);
    ...
    public static _IEngineState create(Dafny.ISequence<ThunderDbStack._IStoredDoc> store, Dafny.ISequence<DocsIndexModel._IEntry> entries, DocsIndexTreap._ITreap treap, Dafny.ISequence<ThunderDbStack._IQueryRuntime> queries, BigInteger nextQueryId) {
      return new EngineState(store, entries, treap, queries, nextQueryId);
    }
    public static _IEngineState create_EngineState(Dafny.ISequence<ThunderDbStack._IStoredDoc> store, Dafny.ISequence<DocsIndexModel._IEntry> entries, DocsIndexTreap._ITreap treap, Dafny.ISequence<ThunderDbStack._IQueryRuntime> queries, BigInteger nextQueryId) {
      return create(store, entries, treap, queries, nextQueryId);
    }
      ...
      while ((_23_tick) < (ticks)) {
        ...
        while ((_24_update) < (updatesPerTick)) {
          if ((new BigInteger(((_0_state).dtor_store).Count)).Sign == 1) {
            ...
            _29_picked = Dafny.Helpers.EuclideanModulus(_26_pickSeed, new BigInteger(((_0_state).dtor_store).Count));
            ThunderDbStack._IStoredDoc _30_doc;
            _30_doc = ((_0_state).dtor_store).Select(_29_picked);
            BigInteger _31_updatedDocState;
            _31_updatedDocState = Dafny.Helpers.EuclideanModulus(_28_scoreSeed, _5_range);
            ThunderDbStack._IEngineState _32_stateAfterUpdate;
            Dafny.ISequence<ThunderDbStack._IDownstreamEvent> _33_events;
            BigInteger _34_ignoredQueryId2;
            ThunderDbStack._IEngineState _out16;
            Dafny.ISequence<ThunderDbStack._IDownstreamEvent> _out17;
            BigInteger _out18;
            ThunderDbStack.__default.ProcessItem(_0_state, ThunderDbStack.StreamItem.create_DocChangeItem((_30_doc).dtor_id, ThunderDbStack.MaybeDocState.create_HasState((_30_doc).dtor_state), ThunderDbStack.MaybeDocState.create_HasState(_31_updatedDocState)), out _out16, out _out17, out _out18);
            _32_stateAfterUpdate = _out16;
            _33_events = _out17;
            _34_ignoredQueryId2 = _out18;
            _0_state = _32_stateAfterUpdate;
            _1_summary = ThunderDbStack.__default.UpdateSummary(_1_summary, _33_events);
          }
          _24_update = (_24_update) + (BigInteger.One);
        }
        BigInteger _35_insert;
        _35_insert = BigInteger.Zero;
        while ((_35_insert) < (insertsPerTick)) {
          BigInteger _36_nextRng5;
          BigInteger _37_scoreSeed;
          BigInteger _out19;
          BigInteger _out20;
          ThunderDbStackPerf.__default.NextRand(_2_rng, out _out19, out _out20);
          _36_nextRng5 = _out19;
          _37_scoreSeed = _out20;
          _2_rng = _36_nextRng5;
          BigInteger _38_newDoc;
          _38_newDoc = Dafny.Helpers.EuclideanModulus(_37_scoreSeed, _5_range);
          ThunderDbStack._IEngineState _39_stateAfterInsert;
          Dafny.ISequence<ThunderDbStack._IDownstreamEvent> _40_events;
          BigInteger _41_ignoredQueryId3;
          ThunderDbStack._IEngineState _out21;
          Dafny.ISequence<ThunderDbStack._IDownstreamEvent> _out22;
          BigInteger _out23;
          ThunderDbStack.__default.ProcessItem(_0_state, ThunderDbStack.StreamItem.create_DocChangeItem(_4_nextDocId, ThunderDbStack.MaybeDocState.create_NoState(), ThunderDbStack.MaybeDocState.create_HasState(_38_newDoc)), out _out21, out _out22, out _out23);
          _39_stateAfterInsert = _out21;
          _40_events = _out22;
          _41_ignoredQueryId3 = _out23;
          _0_state = _39_stateAfterInsert;
          _1_summary = ThunderDbStack.__default.UpdateSummary(_1_summary, _40_events);
          _4_nextDocId = (_4_nextDocId) + (BigInteger.One);
          _35_insert = (_35_insert) + (BigInteger.One);
        }
        BigInteger _42_delete;
        _42_delete = BigInteger.Zero;
        while ((_42_delete) < (deletesPerTick)) {
          if ((new BigInteger(((_0_state).dtor_store).Count)).Sign == 1) {
            BigInteger _43_nextRng6;
            BigInteger _44_pickSeed;
            BigInteger _out24;
            BigInteger _out25;
            ...
            _44_pickSeed = _out25;
            _2_rng = _43_nextRng6;
            BigInteger _45_picked;
            _45_picked = Dafny.Helpers.EuclideanModulus(_44_pickSeed, new BigInteger(((_0_state).dtor_store).Count));
            ThunderDbStack._IStoredDoc _46_doc;
            _46_doc = ((_0_state).dtor_store).Select(_45_picked);
            ThunderDbStack._IEngineState _47_stateAfterDelete;
            Dafny.ISequence<ThunderDbStack._IDownstreamEvent> _48_events;
            BigInteger _49_ignoredQueryId4;
            ThunderDbStack._IEngineState _out26;
            Dafny.ISequence<ThunderDbStack._IDownstreamEvent> _out27;
            BigInteger _out28;
            ThunderDbStack.__default.ProcessItem(_0_state, ThunderDbStack.StreamItem.create_DocChangeItem((_46_doc).dtor_id, ThunderDbStack.MaybeDocState.create_HasState((_46_doc).dtor_state), ThunderDbStack.MaybeDocState.create_NoState()), out _out26, out _out27, out _out28);
            _47_stateAfterDelete = _out26;
            _48_events = _out27;
            _49_ignoredQueryId4 = _out28;
            _0_state = _47_stateAfterDelete;
            _1_summary = ThunderDbStack.__default.UpdateSummary(_1_summary, _48_events);
          }
          _42_delete = (_42_delete) + (BigInteger.One);
        }
